...
namespace single
{
    ...
    double resources(double capital, double hours, double A, int gender, par_struct *par)
    {
        double income = utils::wage_func(capital, 1, par) * hours;
        if (gender == man)
        {
            income = utils::wage_func(capital, 2, par) * hours;
        }
        return par->R * A + income;
    }

    double value_of_choice_single_to_single(double capital, double hours, int kids, double C_tot, double M, int gender, double *V_next, par_struct *par,)
    {
        // Flow utility
        ...
        double income = utils::wage_func(capital, gender, par) * hours;
        double a_next = par->R * M + income - C_tot;
        double k_next = capital + hours;

        // V_next interpolation
        double V_next_no_birth = 0.0;
        double V_next_birth = 0.0;

        if (gender == woman)
        {
            int kids_next = kids;
            auto &V_next_vec = sol->Vw_single[t + 1][kids_next];
            V_next_no_birth = interp_2d(par->grid_Aw, par->Kw_grid, V_next_vec, a_next, k_next);
            ...
            {
                kids_next = kids + 1;
                auto &V_next_birth_vals = sol->Vw_single[t + 1][kids_next];
                V_next_birth = interp_2d(par->grid_Aw, par->Kw_grid, V_next_birth_vals, a_next, k_next);
            }
        }
        else
        {
            int kids_next = kids;
            auto &V_next_vec = sol->Vm_single[t + 1][kids_next];
            V_next_no_birth = interp_2d(par->grid_Am, par->km_grid, V_next_vec, a_next, k_next);
            ...
            {
                kids_next = kids + 1;
                auto &V_next_birth_vals = sol->Vm_single[t + 1][kids_next];
                V_next_birth = interp_2d(par->grid_Am, par->km_grid, V_next_birth_vals, a_next, k_next);
            }
        }

        double EV_next = par->p_birth * V_next_birth + (1 - par->p_birth) * V_next_no_birth;
    ...
    double objfunc_single_to_single(unsigned n, const double *x, double *grad, void *solver_data_in)
    {
        ...
        int kids = solver_data->kids;
        double M = solver_data->M;
        par_struct *par = solver_data->par;

        return -value_of_choice_single_to_single(C_tot, M, gender, capital, hours, kids, solver_data->V_next, par);
    }

    EXPORT void handle_liquidity_constraint_single_to_single(int t, double capital, double hours, int kids,  int gender, double *m_vec, double *C_tot, double *C_priv, double *C_pub, double *V, double *EV_next, sol_struct *sol, par_struct *par)
    {
        ...
        {
            double M_now = resources(grid_A[iA], capital, hours, gender, par);
            ...
            if (M_now < m_vec[0])
            {
                ...
                // c. Calculate value
                V[iA] = value_of_choice_single_to_single(C_tot[iA], M_now, gender, capital, hours, kids, EV_next, par);
            }
        }
    }

    void do_upper_envelope_single_to_single(int t, int gender, double capital, int kids, double hours, double *m_vec, double *c_vec, double *v_vec, double *C_tot, double *C_priv, double *C_pub, double *V, double *EV_next, sol_struct *sol, par_struct *par)
    {
        // Unpack
        double *grid_A = par->grid_Aw;
        if (gender == man)
        ...
        for (int iA_pd = 0; iA_pd < par->num_A_pd; iA_pd++)
        {
            ...
            for (int iA = 0; iA < par->num_A; iA++)
            {
                ...
                double M_now = resources(grid_A[iA], gender, capital, hours, par);
    ...
    void EGM_single_to_single(int t, int gender, sol_struct *sol, par_struct *par)
    {
        ...
        for (int iA_pd = 0; iA_pd < par->num_A_pd; iA_pd++)
        {
            ...
            /// e. value
            V_pd[iA_pd] = value_of_choice_single_to_single(C_tot_pd[iA_pd], M_pd[iA_pd], gender, capital[iA_pd], hours[iA_pd], kids, &EV[idx_next], par);
        }

        // 3. liquidity constraint
        handle_liquidity_constraint_single_to_single(t, gender, kids, hours, capital, M_pd, &C_tot[idx], &C_priv[idx], &C_pub[idx], &V[idx], &EV[idx_next], sol, par);
    ...
    void solve_single_to_single(int t, sol_struct *sol, par_struct *par)
    {
        ...
        if (t == (par->T - 1))
        {
            for (int iA = 0; iA < par->num_A; iA++)
            {
                ...
                sol->Cw_tot_single_to_single[idx] = resources(Aw, capital, hours, woman, par);
                sol->Cm_tot_single_to_single[idx] = resources(Am, capital, hours, man, par);
            ...
            if (par->do_egm)
            {
                ...
                #pragma omp parallel num_threads(par->threads)
                {
                    ...
                    for (int iA = 0; iA < par->num_A; iA++)
                    {
                        ...
                        double Mw = resources(Aw, capital, hours, woman, par);
                        double Mm = resources(Am, capital, hours, man, par);
    ...
    void solve_couple_to_single(int t, sol_struct *sol, par_struct *par)
    {
        #pragma omp parallel num_threads(par->threads)
        {
            ...
            for (int iA = 0; iA < par->num_A; iA++)
            {
                ...
                sol->Hw_couple_to_single[idx] = sol->Hw_single[idx];
                sol->Hm_couple_to_single[idx] = sol->Hm_single[idx];}
        }
    }
